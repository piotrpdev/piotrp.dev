<script>
	export let src, alt, arr, cc, wc, art, wcu, dg, arts, white;
	import ExtLink from '~/components/widgets/svelte/ExtLink.svelte';
</script>

<div class="media-container">
	<figure>
		<img loading="lazy" decoding="async" class={`media ${white ? 'bg-white' : ''}`} {alt} {src} />
		<figcaption>
			<i>
				<slot />
				{#if dg}
				...
				{/if}
				{#if arts}
					<span>Taken from <ExtLink href={arts}>this article</ExtLink>.</span>
				{/if}
				{#if art}
