...
image: ~/assets/images/blog/windows-screenfetch/thumb.png
category: Projects
tags:
  - desktop
  - script
...
import speedData from '~/assets/images/blog/windows-screenfetch/data.json'
import SpeedTable from '~/components/widgets/svelte/SpeedTable.svelte'
import DoubleBar from '~/components/widgets/svelte/DoubleBar.svelte'

export const xCallback = function (value) {
	return this.getLabelForValue(value).substring(4, 12);
};

export const [beforeTotals, afterTotals] = ["before", "after"].map((data) => Object.assign(
...
<div class="hide-on-mobile">
    ...
    <DoubleBar client:only="svelte" title="Windows-screenFetch vs Windows-screenFetch-fast" xCallback={xCallback} yTitle="Speed (ms)" data={totalsBarData} />
</div>
